...
class VersionedDefinitionRef:
    ...
    def __post_init__(self) -> None:
        ...
        if isinstance(self.version, bool) or self.version < 1:
            raise ContractError("definition version must be a positive integer")
...
class MembershipProjection:
    ...
    def __post_init__(self) -> None:
        ...
        if self.authorization_version < 1:
            raise ContractError("authorization version must be positive")

        if self.entitlement_version < 1:
            raise ContractError("entitlement version must be positive")
...
class AdapterGrantBinding:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            isinstance(self.entitlement_definition_version, bool)
            or self.entitlement_definition_version < 1
        ):
            raise ContractError(
                "entitlement definition version must be a positive integer"
            )

        validate_typed_id(
            self.policy_definition_id,
            "pol",
        )
        ...
        if (
            ...
            or self.policy_definition_version < 1
        ):
            raise ContractError(
                "policy definition version must be "
                "a positive integer"
            )
...
class HydrationProjection:
    ...
    def __post_init__(self) -> None:
        ...
        if self.configuration_version < 1:
            raise ContractError("hydration configuration version must be positive")
...
class ConsumerHandoffProjection:
    ...
    def __post_init__(self) -> None:
        ...
        if self.authorization_version < 1:
            raise ContractError(
                "handoff authorization version must be positive"
            )

        if self.entitlement_version < 1:
            raise ContractError(
                "handoff entitlement version must be positive"
            )

        if self.hydration.organization_id != self.organization_id:
            raise ContractError(
                "handoff hydration organization mismatch"
            )

        if self.hydration.tenant_id != self.tenant_id:
            raise ContractError(
                "handoff hydration tenant mismatch"
            )
