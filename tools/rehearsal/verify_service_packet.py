#!/usr/bin/env python3
"""Offline verifier for the four-service safe-pause rehearsal packet."""
...
def main() -> int:
    ...
    packet = json.loads((packet_dir / "service-evidence-packet.json").read_text(encoding="utf-8"))
    ...
    if packet.get("production_endpoint_contact_count") != 0:
        failures.append("production endpoint contact count")
    if not packet.get("all_unknowns"):
        failures.append("unknown preservation")
    if packet.get("service_manifest", {}).get("services") != ["minio", "nats", "postgres", "zitadel"]:
        failures.append("service manifest")

    report = {
        "schema": "anarchi.anar-core.service-evidence-verification.v1",
